...
要約作成は同期完了待ちではなく、状態保存型で進める。APIは最初にPDF本文抽出またはTextractジョブ開始を行い、以後の状態確認でOCR完了確認、OCRテキスト保存、要約生成を進める。表示用の詳細要約は `summaries/` に保存し、RAG 用の短い文書は `kb/` に保存して Bedrock Knowledge Base の同期対象に含める。
