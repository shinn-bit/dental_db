...
VISION_PROMPT = """\
この画像を見て、2段階で判定してください。

【ステップ1: スキップ判定】
次のいずれかに該当すれば、「SKIP」とだけ出力して終了してください：
- 章扉・PART扉・セクション扉・プロローグ（タイトル文字・章番号・PART番号だけのページ）
- 図表も写真も含まれていないページ
- 出版社ロゴ・著作権表示・奥付・広告
- キャラクターや人物の飾りイラストのみ（内容説明なし）
- 余白・背景・装飾だけ

【ステップ2: 説明】
スキップ対象でなければ、内容を150字以内の日本語で説明してください。
以下のルールを必ず守ること：
- マークダウン記法（**、##、- 等）は一切使わない
- 箇条書きにせず、ひとつながりの文章で書く
- 「この画像は〜」「画像には〜」などの前置きを書かない
- 図や表が何を示しているか、数値や比較内容を具体的に書く

出力は「SKIP」か説明文のみ。理由・前置き・補足は不要。\
"""
