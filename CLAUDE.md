...
Lambdaコードは `lambda/summary_worker/handler.py`。
変更後はGit pushだけでなく、AWS Lambdaへ明示的に反映する。
...
aws lambda update-function-code `
  ...
  --zip-file fileb://lambda/summary_worker.zip `
  --profile dental-dev `
...
aws lambda get-function-configuration `
  ...
  --profile dental-dev `
  --region ap-northeast-1 `
  --query "{LastUpdateStatus:LastUpdateStatus,LastModified:LastModified,CodeSha256:CodeSha256}"
